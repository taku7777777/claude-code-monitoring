...
- **キャッシュ原則** = リクエスト先頭からの完全一致（prefix）に効く。TTL は 5分/1h の2種で、
  Claude Code の実測課金は 1h（書込 2.0x — pricing.yaml 校正）。
  下げる行動は「途切れさせる」系（放置・/clear・設定変更・モデル切替・compaction）。
  仕組みの詳細は [0-fundamentals/01](../0-fundamentals/01-cost-model.md)
