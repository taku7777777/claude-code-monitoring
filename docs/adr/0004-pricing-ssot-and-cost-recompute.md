...
- **発効日つき価格改定**: Sonnet5 は 2026-09-01 以降値上げ予定（best-effort 予測）。日付を
  無視した単一価格では、値上げ前後をまたぐ期間で誤る。
...
- 価格表は「モデル ID × 発効日」で管理。`price_changes[].from` で発効日つき改定を表現し、
  Sonnet5 の 2026-09 値上げは**日付で解決**する。
...
- **`pricing.yaml` の価格・係数はすべて best-effort（2026-07 調査値）**であり、実際の請求とは
  ...
  README に明記）。Sonnet5 値上げも公式告知で要確認の予測値。
